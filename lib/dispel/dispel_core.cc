...
QString currentNodeUUID() {
    ...
    auto value = DEFAULT_SYSTEM_ADDRESS;
    if (nodeID.keys().length() == 0) {
        logDebug() << nodeID.keys();
        logFatal() << "No default node identifier found. No identifier for default IP:" << DEFAULT_SYSTEM_ADDRESS;
    } else
        Q_FOREACH(auto key, nodeID.keys()) {
            logDebug() << "key:" << key;
            if (key == DEFAULT_SYSTEM_ADDRESS) {
    ...
    return value; /* NOTE: will return uuid for 127.0.0.1 host (DEFAULT_SYSTEM_ADDRESS) as current system */
}
