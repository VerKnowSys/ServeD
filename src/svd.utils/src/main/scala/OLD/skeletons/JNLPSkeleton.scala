...
package com.verknowsys.served.utils.skeletons
...
class JNLPSkeleton(var mainClass: String,
    ...
    def this(mainClass: String,
             ...
             codebaseURL: String,
             jnlpFileName: String,
             jars: List[String]) = this (mainClass, title, codebaseURL, jnlpFileName, jars, "", "", "", "", "")
    ...
    def getJNLP =
        <jnlp spec="1.0+" codebase={codebaseURL} href={jnlpFileName}>
            ...
            <resources>
                    <j2se version="1.6+" java-vm-args={paramsJVM}/>{for (jar <- jars)
            yield
