...
class SvdUtilsTest extends Specification with Logging {
    ...
    "chmod should change permissions and count files in given folder properly" in {
        try { 
          SvdSystemManagerUtils.chmod("/tmpfdsa/dupadsf_327/xfdsayz", 0777, true)
        ...
        FileUtils.touch("/tmp/dupa_32745923/dupa011")
        FileUtils.touch("/tmp/dupa_32745923/dupa003")
        SvdSystemManagerUtils.chmod("/tmp/dupa_327", 0777, true) must beTrue
        SvdSystemManagerUtils.chmod("/tmp/dupa_327", 0777, false) must beTrue
        SvdSystemManagerUtils.chmod("/tmp/dupa_327/abc", 0777, true) must beTrue
        SvdSystemManagerUtils.chmod("/tmp/dupa_327/abc", 0777, false) must beTrue
        SvdSystemManagerUtils.chmod("/tmp/dupa_327/xyz", 0777, true) must beTrue
        SvdSystemManagerUtils.chmod("/tmp/dupa_327/xyz", 0777, false) must beTrue
        FileUtils.touch("/tmp/dupa_32745923/dupa011")
        SvdSystemManagerUtils.chmod("/tmp/dupa_32745923/dupa011", 0111, false) must beTrue
