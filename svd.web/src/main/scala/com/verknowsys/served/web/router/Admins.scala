...
class Admins extends Plan with Database with Logging with SvdUtils {
    ...
    def gatherAllProductsAvailable = {
        val letters = SpatialLetters(db)
        val stickers = Stickers(db)
        val tshirts = Clothings(db)
        letters ++ stickers ++ tshirts
    }
    ...
    def intent = {
        ...
        case req @ GET(Path(Seg("productlist" :: Nil))) =>
            ...
            implicit val bindings: List[Binding] =
                Binding(name = "clothings", className = "List[com.verknowsys.served.web.merch.Clothing]") ::
